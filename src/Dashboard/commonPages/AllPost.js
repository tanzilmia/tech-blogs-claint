...
const AllPost = () => {
  ...
  return (
    <div>
        ...
        <Tbody>
          {myposts?.map((mypost) => (
            <Tr key={mypost._id} className="bg-gray-100 text-gray-800">
              ...
              <Td className="py-2 px-4 border text-sm">
                {" "}
                <button onClick={() => handleEdite(mypost._id)}>
                  <BsFillPenFill />
                </button>{" "}
              </Td>
              <Td className="py-2 px-4 border text-sm">
      ...
      <button> <Link to ={user.role === "admin" ? "/dashboard/create-post" : "/authorPannel/create-post"}  className='flex items-center'> Create Post</Link> </button>
      </>
      }
    </div>
  );
